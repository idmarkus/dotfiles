...
class Stitch:
    ...
    def mkmasks(self, maskshape, exp=2):
        ...
        # Premult and convert
        maskx = cv.cvtColor((255. * maskx).astype(np.uint8), cv.COLOR_GRAY2BGR)
        masky = cv.cvtColor((255. * masky).astype(np.uint8), cv.COLOR_GRAY2BGR)
        return maskx, masky
...
if __name__ == "__main__":
    ...
    # pil = Image.fromarray(testim)
    pil = Image.open("testim_lr.png")
    stitch = Stitch(pil, (tilesize_h, tilesize_w), (overlap_h, overlap_w))
    blend = stitch.blend(exp=5)
    blend = np.asarray(blend)
    h, w = blend.shape[:2]
    blend = cv.resize(blend, (w // 4, h // 4))
