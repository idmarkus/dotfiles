...
obj = C.object
if obj.type != "MESH":
    raise Exception("Active object must be mesh.")
...
def topofaceedge(obj, center_thresh=0.0001, transform=False):
    ...
    #        print(v0_0.index, nv0_0.index)
    #        print(v0_1.index, nv0_1.index)
    #        print(v1_0.index, nv1_0.index)
    #        print(v1_1.index, nv1_1.index)
    ...
    #
    #    def _transform_topo(vi):
    #        oi = topo[vi]
    #        nco = obj.data.verti.co
    #        print(f"{vi} : [{obj.data.vertices[vi].co}, {topo[vi]} : [{nco}]")
    #        nco.x = -nco.x
    #        obj.data.vertices[vi].co = nco
    #
    #    _transform_topo(20004)
    #
    #    for li in ls:
    #        ri = topo[li]
    #        nco = bm.verts[ri].co.copy()
    #        nco.x = -nco.x
    #        bm.verts[li].co = nco
    #
    #
    ...
    #        ri = topo[li]
    #        if ri not in rs:
    #            print(f"topo[{li}] = {ri} not in RS")
    #        nco = bm.verts[topo[li]].co
    #        nco.x = -nco.x
    #        if li == 18956:
    #            print(topo[li], nco)
    #        bm.verts[li].co = nco
    # obj.data.vertices[li].co = nco
    #
    bm.to_mesh(obj.data)
    bm.free()
    return topo
...
# print(f"Unfound: {n_unfound}")
# print(unfound)


# for i, o in enumerate(ret):
#    if o == -1:
#        print(f"Match not found for v{i}")
...
# for idx_l, idx_r in topo.items():
#    mirror = obj.data.vertices[idx_r].co.copy()
#    mirror.x = -mirror.x
#    obj.data.vertices[idx_l].co = mirror

# dt = perf_counter() - t
# print(f"{dt} s")

# tmap = topomap(obj)
# for l, r in tmap.items():
#    mirror_co = obj.data.vertices[r].co.copy()
#    mirror_co.x = -mirror_co.x
#    obj.data.vertices[l].co = mirror_co

# print(len(idx_l), len(idx_r))
# poly_r = [p.index for p in obj.data.polygons if p.center.x > 0]
# print(len(poly_r))

# t = perf_counter()

# dist_l = np.array([p.magnitude for p, _ in poly_l])
# dist_r = np.array([p.magnitude for p, _ in poly_r])

# sort_l = np.argsort(dist_l)
# sort_r = np.argsort(dist_r)

# for i, idx_l in enumerate(sort_l):
#    idx_r = sort_r[i]
#    pl = poly_l[idx_l]
#    pr = poly_r[idx_r]
#    cL, piL = pl
...
# print(len(poly_l), len(poly_r))

# nvertices = len(obj.data.vertices)

# center_vs = [v.index for v in obj.data.vertices if math.fabs(v.co.x) <= 0.0001]
# n_indices = (len(obj.data.vertices) - len(center_vs)) // 2

# right_vs = [v.index for v in obj.data.vertices if v.co.x > 0.0001]
# bindex = np.full((nvertices), False)
#    
# print(len(center_vs))
# print(n_indices)
# print(len(right_vs))
...
#    
#    target = p.copy()
#    target.x = math.fabs(p.x)
#    
#    d = 100
#    idx = -1
#    for v1 in obj.data.vertices:
#        p1 = v1.co
#        if p1.x <= 0: continue
#        d1 = (target - p1).magnitude
#        if d1 < d:
#            d = d1
#            idx = v1.index
#    
#    if idx == -1:
#        print(f"No mapping found for {vertex.index} at {p}")
#    else:
#        mapping[vertex.index] = idx
#        
# print(mapping.items()[:5]))
#
